...
import { bridgeETHToStarknet, dataBridgeETHFromStarknet, estimateInvokeMaxFee, getAmountTokenStark, privateToStarknetAddress, sendStarknetTX } from './src/starknet.js';
...
(async() => {
    ...
    for (let i = 0; i < wallet.length; i++) {
        ...
        if (index == 0) {
            ...
            await bridgeETHOrbiter('Arbitrum', 'Linea', wallet[i]);
        } else if (index == 11) {
            await bridgeETHOrbiter('zkSyncEra', 'Linea', wallet[i]);
        } else if (index == 12) {
            await bridgeETHOrbiter('Linea', 'Arbitrum', wallet[i]);
        }
